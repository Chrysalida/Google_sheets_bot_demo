...
from BB_config import TOKEN, Logpath, \
                      ...
                      Medic_range, Tlg_IDs_range, admin_ID
import telebot
...
def Name_handler(message):#получили от человека его фамилию
    ...
    if result==None:
    ...
    elif len(result)==1:#result is like: {2: 'Аргонова Ольга'}
        ...
        def Yes_No_handler_name(call):
          ...
          #result is like: {2: 'Аргонова Ольга',  4: 'Анохин Руслан'}
...
#TEST DISPATCHER
    ...
    key_coord=types.InlineKeyboardButton(text="координаторам",callback_data='coords');
    key_everyone=types.InlineKeyboardButton(text="всем",callback_data='everyone');

    dispatch_keyboard.add(key_Med,key_nonMed,key_coord,key_everyone)
    ...
    def Dispatcher(call):
        ...
        if call.data=='med':
            ...
            bot.send_message(call.from_user.id,'Формирую список координаторов...')
            dispatch_list=Lists_to_dispatch.lists_to_dispatch()
            dispatch_list=dispatch_list['Coords']
